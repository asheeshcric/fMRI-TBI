...
class Custom3D(nn.Module):
    def __init__(self, params):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Conv3d(self.nc, self.ndf, 5, 2, 1, bias = False),
            nn.ReLU(True),
            ## state size. (ndf) x 26 x 31 x 24
        )
        self.conv2 = nn.Sequential(
            nn.Conv3d(self.ndf * 1, self.ndf * 2, 4, 2, 1, bias = False),
        ...
        self.conv4 = nn.Sequential(
            nn.Conv3d(self.ndf * 4, self.ndf * 4, 4, 2, 1, bias = False),
            nn.BatchNorm3d(self.ndf * 4),
            nn.ReLU(True),
            ## state size. (ndf*2) x 3 x 3 x 3
        )
        self.conv5 = nn.Sequential(
            nn.Conv3d(self.ndf * 4, self.ndf * 2, 3, 1, 0, bias = False),
            nn.ReLU(True),
        )
